...
# Mount subvolumes
mount -o rw,noatime,compress=zstd:3,ssd,discard=async,space_cache=v2,subvol=@ /dev/"$disk"3 /mnt
mkdir -p /mnt/{boot,home}
mount -o rw,noatime,compress=zstd:3,ssd,discard=async,space_cache=v2,subvol=@home /dev/"$disk"3 /mnt/home
...
diskid=$(blkid | grep "$disk"3 | grep -o 'PARTUUID="[^"]*"' | sed 's/PARTUUID="//;s/"//')
